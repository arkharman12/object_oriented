...
<body>
    ...
    if ($status=="Student") {
      ...
      // Make sure user doesn't get through without passing the requirements
        ...
        $msg2 = "<br /><span style=\"color:red\">You must agree to the Terms & Policies</span><br />";
      } // Put select ok condition here if needed (just like agreeok)
			else {
        ...
        // Header ("Location:lab4info.php?");	

				//now send the email to the username registered for activating the account
        $code = randomCodeGenerator(50);
        $subject = "Email Activation";
                  
        $body = 'Welcome '.$fn.' '.$ln.'!<br/> <br/>
          Please click on the link below to activate your account. <br/>
          http://corsair.cs.iupui.edu:24061/lab4/lab4info.php?a='.$code;
           
        $mailer = new Mail();
        if (($mailer->sendMail($em, $fn, $subject, $body))==true)
          $msg4 = '<span style="color:green"><b>Thank you for registering! A welcome message has been sent to the e-mail address you have just registered.</b></span><br /><br />';
        else $msg4 = "Email not sent. " . $em.' '. $fn.' '. $subject.' '. $body;
    ...
    <div class="container">
      <form class="well form-horizontal" action="lab4.php" method="post" id="reach_form">
        <fieldset>
          ...
          <div class="form-group">
            ...
            <div class="col-md-4">
              <div>
                <label>
                  <input type="checkbox" name="status" value="Student" <?php print $student_checked; ?> /> Student
                </label>
              </div>
              <div>
                <label>
                  <input type="checkbox" name="status" value="Faculty" <?php print $faculty_checked; ?>  /> Faculty
                </label>
			  </div>
			  <div>
                <label>
                  <input type="checkbox" name="status" value="Staff" <?php print $staff_checked; ?>  /> Staff
                </label>
              </div>
            </div>
          </div>
